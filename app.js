...
function _logProfits(price) {
	...
	logColor(
		colors.gray,
		`Balance: ${m1Balance} ${MARKET1}, ${m2Balance.toFixed(
			2
		)} ${MARKET2}, Current: ${parseFloat(
			m1Balance * price + m2Balance
...
async function _buy(price, amount) {
	if (
		...
		log(`
            Buying ${MARKET1}
            ==================
            amountIn: ${parseFloat(BUY_ORDER_AMOUNT * price).toFixed(
							2
						)} ${MARKET2}
...
async function _sell(price) {
	...
	if (toSold.length > 0) {
		...
		if (
			totalAmount > 0 &&
			parseFloat(store.get(`${MARKET1.toLowerCase()}_balance`)) >= totalAmount
		) {
			log(`
                Selling ${MARKET1}
                =================
                amountIn: ${totalAmount.toFixed(2)} ${MARKET1}
                amountOut: ${parseFloat(totalAmount * price).toFixed(
									2
								)} ${MARKET2}
            `)
		...
		try {
			...
			if (mPrice) {
				...
				if (marketPrice < startPrice) {
					...
					logColor(
						colors.red,
						`Losers: -${parseFloat(percent).toFixed(3)}% ==> -$${parseFloat(
							factor
						).toFixed(4)}`
					)
					...
					logColor(
						colors.green,
						`Gainers: +${parseFloat(percent).toFixed(3)}% ==> +$${parseFloat(
							factor
						).toFixed(4)}`
					)
					store.put('percent', `+${parseFloat(percent).toFixed(3)}`)
				...
				const orders = store.get('orders')
				if (orders.length > 0) console.log(orders[orders.length - 1])
			}
		} catch (err) {}
